...
3. Update the session note in place:
   ...
   - Overwrite (create if absent) a `## Handoff` section with these subsections, written so a fresh session with no memory of this conversation can act immediately:
     ...
     - `### Pedagogy` *(for every session that ran a tutorial — routine `4a`, code-review `4b`, issue-work `4c`, feature `4d`; omit only for a session with genuinely no tutorial)* — the checkpoint that lets `/resume-work` re-enter the teaching flow without losing a passed gate or re-quizzing it. Because one session can teach+gate more than one issue (TS-7 re-arms on each materially-different issue worked), this is a **per-work-item list**, not a single gate — one row per item worked this session:
       ```
       - item: <short id/desc> | procedure: complete|partial(…)|none|n/a | gate1: passed|open | gate2: passed|open|n/a | pos: step N of M|not-started|n/a | status: active|done|opted-out|parked
         claims: G1: <learner sentence>; <learner sentence> | G2: <delta or empty>
       ```
       The five pipe fields stay the authority for passed/open. **`claims:`** is an optional indented sibling (4a/4c/4d only — omit for 4b): learner-produced claim sentences written **at Gate 1/2 pass** by `/start-work` (or resume re-entering those phases). **On pause: copy `claims:` verbatim if present; if absent, leave absent — never invent claims from chat.** Completing hybrid P5 does not write `claims:` or flip `gate1`/`gate2` to `passed`. Plus the session `type:`, and — if the pause happened during `/end-work`'s sandbox — the sandbox path (the note's frontmatter already carries `sandbox: in-progress`). **`procedure:` is a compliance receipt** for whether §Procedure ran when the gate was taught — not a skip-re-present flag. A `none` or `partial` value means the gate must not be recorded as passed. Absent `procedure:` on legacy rows = `unrecorded`; do not invalidate an existing `gate1: passed`. Absent `claims:` on a legacy or pre-patch row does **not** invalidate an existing `gate1: passed`. **Presentation is deliberately NOT recorded as durably done:** TS-7 requires each fresh session to re-present a still-open gate's material before running it, so there is no `presented:` flag to go stale — `claims:` is the object to re-teach, not a skip-re-present bit. **Per-type notes:** `4c`/`4d` use all fields + `claims:`; **`4a`** the same, but gates were run *proportionately* (`procedure: n/a` when escaped); **`4b`** gates diff-comprehension only — `gate2: n/a`, `pos: n/a`, no `claims:`. When a pause happens *inside* `/end-work`'s sandbox, `/end-work` writes this block itself rather than routing through `/pause-work`. See the Phase 1–4 flow in `start-work.md`, TS-7 in `{{TEACHING_STANDARD_PATH}}`, and the sandbox step in `end-work.md`.
