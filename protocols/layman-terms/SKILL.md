...
name: layman-terms
description: >
  Rewrite any given text into plain language that's understandable in a single read, without
  sacrificing length beyond what clarity requires. Every claim must trace to a premise or fact
  already in the source — never invented to smooth the prose. No unexplained jargon or
  buzzwords; replace or define them inline. This is a clarity rewrite, not a summary.

  Trigger on: "explain this simply", "make this understandable", "in plain English", "no jargon",
  "in layman's terms", "ELI5 but rigorous", "cut the buzzwords", "dumb this down" — all in the
  sense of clarity, not content loss; this skill never drops or summarizes substance.
---
...
domain expert. Keep the source's language — this skill doesn't translate.

This skill rewrites prose. It does not apply to code, data tables, or diagrams — if the input
...
1. **Every claim must trace to something already in the source.** If the source states a fact or
   ...
   (step 5).
2. **No unexplained jargon or buzzwords.** Check every sentence against `denylist.txt` in this
   protocol folder (sidecar after a command install — see `protocols/README.md`; never hardcode a
   harness home path) and replace hits with their plain equivalents. A real technical term or
