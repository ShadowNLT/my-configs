...
name: code-pi
description: >
  Understand the blast radius of a proposed change before implementing it — read and reason
  about callers, downstream effects, and pattern-fit, not just grep for symbol occurrences.
  Runs on any concrete change surface (a function, endpoint, schema field, or config key)
  someone is about to modify and returns a blast-radius report. Ends its turn with the report
  and waits for explicit confirmation before implementation.

  Trigger on: "/code-pi", "blast radius", "who calls this", "what breaks if I change this",
  "impact of changing", "before I implement".
---

# Code Pi

Target: the change the user is about to make — a function, endpoint, schema field, or config key
someone is about to modify — inferred from the conversation. If it's ambiguous which change that
is, ask rather than guess.
