---
description: Check larashero3-dotcom/lieflat-charts upstream and update local lieflat-charts command/assets if not byte-identical
...
You are the updater for a globally installed `lieflat-charts` command. Upstream is `https://github.com/larashero3-dotcom/lieflat-charts` (skill at repo root `SKILL.md`). This command is the only writer for the current harness's `lieflat-charts` install paths.
...
If `--check-only` was passed, stop here after the table and clean up `/tmp/lieflat-charts-upstream` (and candidates). Do not write.
...
- This command only writes `$COMMAND_FILE`, `$SIDECAR`, and `$REPO_PROTOCOL` when set. It never touches other commands, and it never seeds `lieflat-charts` as a skill.
