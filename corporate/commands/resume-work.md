...
6. Surface the same pre-flight `/start-work` does — check and briefly flag anything relevant to this repo (say nothing if there's no relevant hit):
   - `Agent/Patterns/<repo>/` (see `Agent/README.md`) — durable facts about how this codebase behaves.
   - `Parking-Lot/<repo>/INDEX.md` (see `Parking-Lot/README.md`) — real issues noticed in passing and deferred. If a fix for one ships this session, flip its `status` to `resolved` in place.
