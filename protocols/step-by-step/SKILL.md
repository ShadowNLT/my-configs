...
description: >-
  ...
  result, and stops — advancing only on an explicit confirmation. Progress is
  tracked in a durable state file so it survives context loss. Ends when the
  goal's done-condition is met, the user stops it, or a bounded retry budget is
  exhausted.
  Trigger on: "/step-by-step", "walk me through this one step at a time",
  "step by step", "one step at a time", "don't run ahead — guide me".
---

# step-by-step

Guide the user to a goal **one step at a time**. The rule that defines this
skill: **never advance to the next step until the user has explicitly confirmed
...
## 0. Durable state (do this — the skill breaks without it)
...
- **One step per turn.** Never batch (unless §-top grouping was agreed) or run
