...
7. Continue the work described in `### What's next`.
   ...
   **If this is a tutorial session (`4a`/`4b`/`4c`/`4d`) with a `### Pedagogy` checkpoint, re-enter the teaching flow at that point rather than restarting it** (see the Phase 1–4 flow in `start-work.md` and the sandbox step in `end-work.md`). For **4a** resume the proportionate gates/tutorial as recorded; for **4b** resume the diff-comprehension gate (there is no Phase 4). **`{{TEACHING_STANDARD_PATH}}` applies to the resumed teaching exactly as in the original session** — §Procedure + TS-1, TS-2, TS-3, TS-4, TS-5, TS-6, **TS-7 (re-present still-open gate material *this session* before running it)**:
   - **Do not re-quiz a `passed` gate** — treat `gate1: passed`/`gate2: passed` as already cleared (a `4b` session has only `gate1`; its `gate2` is `n/a`).
   - **Multiple open `item:` rows:** ask which to resume — never pick by recency (same rule as pause/end matching). Cross-item callbacks are a miss.
   - **Resume an `open` gate (4a/4c/4d) — re-present then ladder (TS-7):**
     - If the row has `claims:` and `procedure: complete`: **short P5** — for each stored claim, rebuild the TS-1 derivation **this session** (common knowledge / vault link / a prior claim in the same list). The sibling is the destination list, not the lesson; do not read it aloud and gate; do not re-run picture P1–P4 from ordinary-adult floor. Then run the open gate on those claims (Gate 1 = map + claims bar; Gate 2 = callbacks to G1 + solution claims). Sketch may illustrate; sketch recitation is still a miss.
     - If `gate1: open` / `gate2: open` and `procedure: none|partial(…)`: treat the prior teach as defective — re-run §Procedure for that gate's material, then gate.
     - If `gate1: passed`, `gate2: open`, and **`claims:` is absent** (legacy / pre-patch / pause never wrote it): **do not invent claims from `## Journal` or chat.** Stop and ask: **re-teach Gate 1 this session** (full picture §Procedure, then Gate 1) **or** run Gate 2 **without callbacks** and record `procedure: partial(no-climb-receipt)`. In-flight sketch-era passes stay passed; they have no callback source until that choice.
     - If `gate1: open` and `claims:` absent: run the picture §Procedure (or proportionate 4a teach), then Gate 1.
   - Legacy rows with no `procedure:` field (`unrecorded`) do not invalidate an existing `gate1: passed`. Resume the tutorial at the recorded `step N of M`; don't restart from step 1.
   - **Multiple work-items in one `### Pedagogy` block** (a session that taught+gated issue A, then re-armed for issue B — see TS-7): re-enter **each item at its own recorded state**; a `passed`/`done` item stays cleared, an `open` item re-presents-then-gates per the rule above, an `opted-out` item is not re-taught. Match `gate1: passed` and `claims:` to the item they belong to — never to the wrong issue.
   ...
   - **If the `### Pedagogy` block is absent** (a legacy paused note from before this checkpoint existed), say so and reconstruct minimally from the `## Journal` rather than assuming which gates were passed — still **do not invent `claims:`**.
