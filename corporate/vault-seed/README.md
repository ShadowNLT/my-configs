...
Vault root. Top-level folders: `Personal/`, `Vivenu/`, `Work/` (session tracking, see `Work/00-How-Work-Tracking-Works.md`), `Agent/` (the agent's corner — see `Agent/README.md`, the user never reads or edits it).
...
Applies to anything the agent writes in this vault meant for the user to read. Does not apply to `Agent/` — that corner is the agent's own operational reference, and the agent uses whatever format serves its own future comprehension there, diagram or not.
...
**Scope note:** `Onboarding/Engine/` is in scope even though the agent mostly maintains it, because its own README says the user can read and edit it like any other note — unlike `Agent/`, which is explicitly hands-off for the user. Confirmed with the user 2026-07-06: the Agent corner means the `Agent/` folder specifically, not a broader "anything the agent mostly writes" rule.
