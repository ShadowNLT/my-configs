...
description: >
  ...
  review log. REASONING mode (no file path) critiques a plan/decision from the conversation and
...
defect-dense figure (60 agents) — or a user-supplied `--max-agents N` — is a stop-and-ask,
never a silent trim.

**Layman-terms rule (every user decision point):** whenever this review stops to put a decision
in front of the user — the no-query prompt (§1.4), ambiguous-path and validation stops (§1),
...
## 1. Parse Arguments, Validate, Select Mode

`$ARGUMENTS` is a free-form string: `"<query>" [file-path ...] [flags]`. One query followed by
several paths applies that query to every doc; multiple quoted queries select the per-query pair
form (each query binds to the paths following it until the next query); a path with no governing
query is reported like a dangling query.

### 1.1 Mode selection
- **Document mode** — one or more file paths are given, or `--doc` is passed.
- **Reasoning mode** — no file path is given, or `--reasoning` is passed. The target is the most
  substantive plan/decision/reasoning proposed in the conversation (prefer the most recent one
  the user is actually deciding on over a trivial aside). **State explicitly which target you
  picked and why.**
- **Ambiguity → stop and ask.** If a token looks like an intended path but does not resolve to a
  readable file (typo, wrong cwd, moved file), do **not** silently fall back to reasoning mode —
  stop and ask whether the user meant a file (and which) or a conversational target. `--doc` with
  no readable path is always an error, not a downgrade.

### 1.2 Input validation (before any work or logging)
Validate up front; on any failure, **stop and report** — do not create a review log or touch
disk:
- Every `--doc` path exists and is readable text. A missing, permission-denied, binary, image, or
  PDF target is reported, not force-read. A target that cannot be embedded in full in a sub-agent
  prompt (working rule: > ~100 KB) forfeits the one-pass guarantee: stop and ask — narrow the
  scope, or run explicitly-declared per-section reviews with the SA consistency sweep logged as
  degraded. Never silently truncate the working copy in any hat/refuter/fresh-eyes prompt.
- `--rounds N` is an integer ≥ 1 (default 6). Zero, negative, or non-numeric is rejected.
- `--until` ∈ {p0, p1, p2} (default p2). Any other value is rejected.
- Unknown flags are reported, not ignored.
- In multi-doc per-query form, every query pairs with a following path; a dangling query is
  reported.

### 1.3 Flags
| Flag | Default | Meaning |
|---|---|---|
| `--doc` / `--reasoning` | inferred | Force the mode instead of inferring from paths |
| `--rounds N` | `6` | Max attack→fix→re-attack cycles per document (document mode; ignored, with a warning, in reasoning mode) |
| `--until p0 \| p1 \| p2` | `p2` | Threshold that defines "clean" and gates which fixes apply by default (§4) |
| `--parallel` / `--solo` | inferred | Force / forbid real parallel sub-agent hats (§5 Round 1) |
...
| `--max-agents N` | unset | Hard per-run agent budget; crossing it mid-run stops and asks (see cost-profile tripwire) |
...
no Round 3, no working-copy mutation, no loop, no escalation (§6), and no `--rounds` iteration —
...
- **Parallel by default.** Unless the target is clearly trivial and reversible, run one Agent
  ...
  not spawn agents or invoke fan-out skills. Force with `--parallel`, forbid with `--solo`.
...
- **Byte-identical** (the approved fix set is exactly the set already applied to the working
